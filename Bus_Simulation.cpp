...
// Add these near other bus position variables
int x2 = -150;  // Position for second bus
int flag2 = 0, flag552 = 0, var2 = 0;  // Control flags for second bus
...
void text1_bus2() { // For second bus
    char string2[] = "TIET BUS(ENC)";
    void* font2 = GLUT_BITMAP_TIMES_ROMAN_24;
    int k;
    glColor3f(0.0, 0.0, 0.0);
    glRasterPos3f(225 + p, 400, 70); // Adjusted Y position
    for(k=0;k<strlen(string2);k++)
        glutBitmapCharacter(font2,string2[k]);
}
void textd()
...
char string[]="Department of Computer Science and Engineering";
...
glRasterPos3f(460,250+100,0);

for(k=0;k<strlen(string2);k++)
        glutBitmapCharacter(font2,string2[k]);

char string3[]="TANYA - 102203548";
void *font3=GLUT_BITMAP_HELVETICA_18;

glColor3f(0.0,0.0,0.0);


glRasterPos3f(460,220+100,0);
...
char string4[]="DHRUVI SINGH-102203536";
void *font4=GLUT_BITMAP_HELVETICA_18;

glColor3f(0.0,0.0,0.0);


glRasterPos3f(460,220+70,0);

for(k=0;k<strlen(string4);k++)
        glutBitmapCharacter(font4,string4[k]);

char string7[]="SMRITI SINGH-102203604";
void *font7=GLUT_BITMAP_HELVETICA_18;

glColor3f(0.0,0.0,0.0);


    glRasterPos3f(460,220+40,0);

    for(k=0;k<strlen(string7);k++)
            glutBitmapCharacter(font7,string7[k]);


char string5[]="SUBMITTED TO: ";
...
void bus_move2() {
    if (x2 < 50) {
        x2 += 4;
        glPushMatrix();
        glTranslatef(x2, 0, 0); // Align with road1 (y=0 for upper road)
        wheel1();
        colorcube();            // Use upper road bus model
        wheel2();
        line();
        text1();                // Use upper road text
        glPopMatrix();
    }
    if (x2 >= 50)
        var2 = 1;
    ...
    if (flag552 == 1) {
        x2 += 1;
        glPushMatrix();
        glTranslatef(x2, -55, 0); // Keep aligned with road1
        wheel1();
        colorcube();
        wheel2();
        line();
...
void display(void)
{
    ...
    glLoadIdentity();
    // In display setup:
    glOrtho(0, 1000, 0, 650, -2000, 1500);// Ensure y=0 is at the bottom
    ...
    if (then == 1)
    {
        ...
        if (flag)
        {
            ...
            text2();
            bus_move();  // Original bus on upper road
            bus_move2();
            // text1();
            glPopMatrix();
